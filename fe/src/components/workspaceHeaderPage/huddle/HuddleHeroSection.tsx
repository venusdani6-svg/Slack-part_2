...
export function HuddleHeroSection({ onClose, onStartHuddle }: Props) {
  return (
    <div className="relative w-full h-74 bg-[#cfe8d6] rounded-lg px-25 flex items-center justify-between">
      ...
      <a
        onClick={onClose}
        className="absolute top-4 right-4 w-6 h-6 flex items-center justify-center text-[#616061] hover:text-[#1d1c1d] cursor-pointer"
      >
        ✕
      </a>
      ...
      <div className="relative mr-40 w-76 h-55 bg-[#cfe8d6] flex items-center overflow-hidden justify-center rounded-lg">
        <Image
          ...
          width={387}
          height={262}
          className="bg-[#cfe8d6] absolute w-98 h-67 left-0 object-cover opacity-[0.9]"
        />
      </div>
    </div>
  );
