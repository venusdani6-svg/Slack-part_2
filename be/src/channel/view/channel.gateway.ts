...
@WebSocketGateway({
  cors: {
    origin: '*', // allow frontend URL here in production
    credentials: true,
  },
})
export class ChannelGateway implements OnGatewayInit {
  ...
  afterInit() {
    this.logger.log('Channel WebSocket Gateway initialized');
  }

  emitChannelCreated(channel: any) {
    this.server.emit('channelCreated', channel);
  }

  emitChannelUpdated(channel: any) {
    this.server.emit('channelUpdated', channel);
  }

  emitChannelDeleted(channelId: string) {
